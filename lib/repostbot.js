...
function Repostbot(controller, worker) {
  ...
  this.worker = worker;
  this.logger = bunyan.createLogger({name: 'repostbot'});
    ...
    if (matches) {
      ...
      controller.storage.channels.get(message.channel, function(err, channel) {
        ...
        self.processMatches(channel, message, matches).then(function(reposts) {
          ...
          self.logger.error("An error occurred whilst processing matches: " + err);
        });
      });
    }
  });
...
Repostbot.prototype.processMatches = function(channel, message, matches) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    for (var i = 0; i < matches.length; i++) {
      ...
      if (linkify.test(url)) {
        self.logger.info("Processing url " + url);
        
        if (url in channel.urls) {
          self.logger.info("Repost detected");
          ...
          self.logger.info("Not a repost, saving URL in JSON storage.");
...
Repostbot.prototype.incrementUserPostCount = function(userid) {
  ...
  return new Promise(function(resolve, reject) {
    self.controller.storage.users.get(userid, function(err, user) {
      self.logger.info("Incrementing count for user " + userid);
  ...
  if (reposts.length > 0) {
    reposts.forEach(function(url, index, array) {
      ...
      self.fetchUser(op.user).then(function(user) {
        ...
        self.logger.info("Replying for reposted URL " + url + "."); 
        ...
        self.logger.error("An error occurred whilst handling the response: " + err);
      });
    });
  }
}
...
Repostbot.prototype.fetchUser = function(id) {
  ...
  return new Promise(function(resolve, reject) {
    self.logger.info("Attempting to fetch user " + id);
    self.controller.storage.users.get(id, function(err, user) {
      if (user && user.name) {
        resolve(user);
      } else {
        self.logger.info(user.id + " is not in storage, calling Slack API.");
