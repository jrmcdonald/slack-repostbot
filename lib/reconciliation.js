    ...
    new Promise(function(resolve, reject) {
      ...
      for (var c in channels) {
        ...
        if (channel.members.indexOf(bot_user) > -1) {
          console.log("INFO: Bot is a member of channel " + channel.id + ", need to perform reconciliation.");
          promises.push(self.reconcileChannel(channel.id));
        }
      }
      
      Promise.all(promises).then(function() {
        console.log("INFO: Finished reconciling known channels.")
      }).catch(function(err) {
        console.log("ERROR: An error occurred reconciling known channels: " + err);      
      }); 
    });
  });
}
...
Reconciliation.prototype.reconcileChannel = function (id) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    self.findLastReconciliation(id).then(function(oldest) {
      if (oldest) {
        console.log("INFO: Reconciliation for " + id + " last occured at " + oldest);
        params.oldest = oldest;
      } 
      
      self.fetchHistory(messages, params).then(function() {
        console.log("INFO: Finished fetching channel history for " + id);
        
        if (messages.length > 0) {
          self.reconcileMessages(messages.reverse(), id).then(function() {
            console.log("INFO: Finished reconciling messages for channel " + id);
            var latest = messages[messages.length - 1].ts;
            
            self.setLastReconciliation(id, latest).then(function() {
              console.log("INFO: Updated last reconcilation date for for channel " + id);
              resolve();
            }).catch(function(err) {
          ...
          console.log("INFO: No new messages since last reconciliation for " + id);
          resolve();
        }
      }).catch(function(err) {
  ...
  return new Promise(function(resolve, reject) {
    self.controller.storage.channels.get(id, function(err, channel) {  
      ...
      if (!(url in channel.urls)){
        console.log("INFO: Reconciling a previously unknown URL <" + url + ">.");
